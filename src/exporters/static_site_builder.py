...
단일 책임: GitHub Pages용 정적 HTML 생성
"""
import shutil
...
class StaticSiteBuilder:
    ...
    jobs.json을 기반으로 GitHub Pages용 정적 HTML을 생성합니다.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            data_dir: 데이터 디렉토리. None이면 기본 경로 사용.
        """
        config = get_config()
        self._output_dir = output_dir or config.docs_dir
    ...
    def build(self) -> None:
        """정적 사이트 생성"""
        # jobs.json 복사
        self._copy_jobs_json()

        # index.html 생성
        self._create_index_html()

        logger.info(f"정적 사이트 생성 완료: {self._output_dir}")

    def _copy_jobs_json(self) -> None:
        """jobs.json 파일 복사"""
        source = self._data_dir / "jobs.json"
        dest = self._output_dir / "jobs.json"

        if source.exists():
            shutil.copy(source, dest)
            logger.debug(f"jobs.json 복사: {source} → {dest}")

    def _create_index_html(self) -> None:
        """index.html 생성"""
        html = self._get_html_template()

        output_file = self._output_dir / "index.html"
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(html)

        logger.debug(f"index.html 생성: {output_file}")
